 ...
 */
export const getPageContentBlockIds = (recordMap: types.ExtendedRecordMap, id?: string): string[] => {
    const rootBlockId = id || Object.keys(recordMap.block)[0];
    ...
    function addContentBlocks(blockId: string) {
        ...
        // eslint-disable-next-line no-restricted-syntax
        for (const blockIdx of content) {
            addContentBlocks(blockIdx);
        }
    }
