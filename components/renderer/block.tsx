import { Box, Container, rem, Title, Center } from '@mantine/core';
...
import { useNotionContext } from './context-root';
import { LinkIcon } from './icons/link-icon';
import { cs, getListNumber, isUrl } from './utils';
import { getBlockCollectionId } from './utils/get-block-collection-id';
import { getBlockIcon } from './utils/get-block-icon';
import { getBlockParentPage } from './utils/get-block-parent-page';
import { getPageTableOfContents } from './utils/get-page-table-of-contents';
import { getTextContent } from './utils/get-text-content';
import { uuidToId } from './utils/uuid-to-id';
    ...
    switch (block.type) {
        ...
        case 'page':
            if (level === 0) {
                ...
                if (fullPage) {
                    ...
                    const hasAside = hasToc;
                    const hasPageCover = pageCover || page_cover;

                    const getDefaultTextGenerator = (subpath: string) =>
                        ({
                            artykul: 'Artykuł',
                            home: 'Strona główna'
                        }[subpath] || subpath);
                    ...
                    return (
                        <Box id={blockId} className={cs(blockId, className)}>
                            ...
                            >
                                ...
                                {block.type !== 'collection_view_page' && (
                                    <Box
                                        sx={(theme) => ({
                                            display: 'grid',
                                            gridTemplateColumns: hasAside ? 'max-content auto' : '1fr',
                                            width: '100%',
                                            gap: rem(16),
                                            position: 'relative',
                                        ...
                                        className={cs(
                                            'notion-page-content',
                                            hasAside ? ('notion-page-content-has-aside' as string) : undefined
                                        )}
                                    >
                                        <Box
                                            p={16}
                                        ...
                                        >
                                            ...
                                            <Container size={720} pt={32}>
                                                ...
                                                {hasPageCover &&
                                                    (pageCover || (
                                                        <Center className="notion-page-cover-wrapper" mx="auto">
                                                            <LazyImage
                                        ...
                                        {hasAside && hasToc && (
                                            ...
                                            // />
                                            // <Aside />
                                            <PageAside toc={toc} hasToc hasAside />
                                        )}
                                    </Box>
                                )}
        ...
        case 'sub_sub_header': {
            ...
            if (indentLevel === undefined) {
                ...
                if (page) {
                    ...
                    const tocItem = toc.find((tocIt: any) => tocIt.id === block.id);
        ...
        case 'callout':
            ...
            return (
                ...
                >
                    <PageIcon block={block} defaultIcon={null} />
